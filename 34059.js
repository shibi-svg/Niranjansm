...
function initSplit() {
    ...
    new SplitText(e, {
        ...
        Array.from(t).map((e => {
            if (e.hasAttribute("split-ran"))
                return;
            let t = new SplitText(e, {
                type: "lines",
                linesClass: "single-line"
            });
            return t.lines.forEach((t => {
                let r = document.createElement("div");
                r.classList.add("single-line-wrap"),
                    "dark" === e.getAttribute("data-load-ghost") && r.setAttribute("data-theme", "dark"),
...
function initTextAnimations() {
    document.querySelectorAll('[data-split="words"]').forEach((e => {
        ...
        const t = e.getAttribute("data-stagger") || .05;
        gsap.set(words, {
            autoAlpha: 0,
            yPercent: 25
        }),
...
function initLoader() {
    ...
    const i = e.querySelectorAll(".load-bar-block")
        ...
        , u = (e.querySelector(".load-cover"),
            e.querySelectorAll(".card-bg-block"))
                ...
                e.setAttribute("data-in-view", "true"),
                    function (e) {
                        e.querySelectorAll("[data-wiggle]").forEach((e => {
                            const t = parseFloat(e.getAttribute("data-wiggle-amplitude")) || 5;
                            gsap.timeline({
                                repeat: -1,
                                repeatRefresh: !0
                            }).to(e, {
...
function initDetectScrollingDirection() {
    ...
    window.addEventListener("scroll", (() => {
        ...
        if (Math.abs(e - t) >= 10) {
            const r = t > e ? "down" : "up";
            document.querySelectorAll("[data-scrolling-direction]").forEach((e => e.setAttribute("data-scrolling-direction", r)));
            const o = t > 50;
            document.querySelectorAll("[data-scrolling-started]").forEach((e => e.setAttribute("data-scrolling-started", o ? "true" : "false"))),
                e = t
        }
    }
    ))
        ...
        document.querySelectorAll("[data-theme-section]").forEach((function (e) {
            ...
            if (o <= r && a >= r) {
                ...
                document.querySelectorAll("[data-theme-nav]").forEach((function (e) {
                    e.getAttribute("data-theme-nav") !== t && e.setAttribute("data-theme-nav", t)
                }
                ))
            }
        }
...
function initMagneticEffect() {
    function e(e) {
        const t = e.currentTarget
            ...
            , i = ((e.clientX - r.left) / t.offsetWidth - .5) * (o / 16)
            , l = ((e.clientY - r.top) / t.offsetHeight - .5) * (o / 16);
        if (gsap.to(t, {
            x: i + "em",
            y: l + "em",
            rotate: "0.001deg",
            ...
            a) {
            const o = ((e.clientX - r.left) / t.offsetWidth - .5) * (n / 16)
                , i = ((e.clientY - r.top) / t.offsetHeight - .5) * (n / 16);
            gsap.to(a, {
                x: o + "em",
                y: i + "em",
                rotate: "0.001deg",
    ...
    window.innerWidth <= 991 || r.forEach((r => {
...
function initTracksLotties() {
    document.querySelector("[data-tracks-grid]").querySelectorAll("[data-tracks-item]").forEach((e => {
        const t = e.querySelector(".tracks-card__visual [data-lottie]");
        if (!t)
            return;
        if (isTablet)
            return void lottie.loadAnimation({
                container: t,
                renderer: "svg",
                loop: !0,
                autoplay: !0,
        ...
        r.addEventListener("DOMLoaded", (() => {
            a = r.totalFrames,
                n.frame = r.currentFrame;
            const l = t.parentElement.querySelector("img");
            l && l.remove(),
                e.addEventListener("mouseenter", (() => {
                    i && i.kill(),
                        n.frame >= a - 1 && (n.frame = 0,
                            r.goToAndStop(0, !0));
                    const e = o ? a : 120
...
function initCanvasGrid() {
    ...
    let n, i, l, s, c = null;
    prefersRM || supportsTouch() || r.addEventListener("mousemove", (e => {
        const t = r.getBoundingClientRect()
            , o = e.clientX - t.left
            , n = e.clientY - t.top;
        let i = s.findIndex((e => o >= e.x && o < e.x + l && n >= e.y && n < e.y + l));
        if (-1 !== i && i !== c) {
            let e = s[i];
...
function initFaq() {
    ...
    e.forEach((t => {
        ...
        r.addEventListener("click", (() => {
            e.forEach((e => {
                e !== t && "open" === e.getAttribute("data-state") && e.querySelector(".faq-button").click()
            }
            )),
                "closed" === t.getAttribute("data-state") ? (t.setAttribute("data-state", "open"),
                    gsap.to(o, {
                        height: "auto",
                        onComplete: () => {
                            ScrollTrigger.refresh()
    ...
    const r = setInterval((() => {
        ...
        if (e && e.contentWindow.document.querySelector('input[type="submit"]')) {
            ...
            const a = document.getElementById("email")
                ...
                , c = l.querySelector('input[name="LEGAL_CONSENT.subscription_type_73561157"]')
                , d = l.querySelector('input[type="submit"]');
            a && s && a.addEventListener("input", (e => {
                ...
                n && c && n.addEventListener("change", (e => {
                    e.target.checked !== c.checked && c.click()
                }
                )),
                i && d && i.addEventListener("click", (e => {
                    e.preventDefault(),
                        d.click(),
...
gsap.registerPlugin(ScrollTrigger, SplitText, CustomEase, Flip, ScrambleTextPlugin),
...
let resizeTimer, isMobile = window.innerWidth < 480, isMobileLandscape = window.innerWidth < 768, isTablet = window.innerWidth < 992, previousWindowWidth = window.innerWidth;
